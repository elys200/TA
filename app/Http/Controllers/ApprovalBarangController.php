...
class ApprovalBarangController extends Controller {
    ...
    public function approval(Request $request, $id) {
        ...
        if( !Hash::check($request->password, Auth::user()->password)) {
            return redirect()->route('approvalbarang') ->with('error', 'Password Salah');
        }
        ...
        $user->notify(new PeminjamanBarangApproveNotification("🔔 Peminjaman barang kamu dengan kode". $peminjaman->code_peminjaman . "telah disetujui!",
                route('statuspeminjamanruangan')));

        return redirect()->route('approvalbarang') ->with('success', 'Status peminjaman barang berhasil diperbarui.');
    }
