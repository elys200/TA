...
class OrmawaController extends Controller
{
    ...
    public function detail($id){
        $ormawa = Ormawa::findOrFail($id); // model tunggal
        $barang = Barang::where('ormawa_id', $id)->first();
        $users = Users::all();
       
        return view('ormawa.detail', compact('ormawa', 'barang', 'users'));
    }
    ...
    public function update(Request $request, $id)
    {
        ...
        $validated = $request->validate([
            'nama_ormawa' => 'required|string|unique:ormawa,nama_ormawa,' . $ormawa->id . '|max:255',
    ...
    public function storeBarang(Request $request, $id)
{
    ...
    if ($request->hasFile('foto_barang')) {
        $validated['foto_barang'] =
            $request->file('foto_barang')->store('barang', 'public');
    }

    // 🔥 langsung lewat relasi
    dd($validated);
    $ormawa->barang()->create($validated);

    return redirect()
        ->route('ormawa.detail', $ormawa->id)
        ->with('success', 'Barang berhasil ditambahkan ke Ormawa.');
}
...
public function destroyBarang($id, $barangId)
{
    ...
    return redirect()
        ->route('ormawa.detail', $ormawa->id)
        ->with('success', 'Barang berhasil dihapus dari Ormawa.');
}
}
